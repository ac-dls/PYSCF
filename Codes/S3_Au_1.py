# Script 3: Field Emission Potential and Stabilization Energies under E-field for CNT (7,5)+Au
...
geom = np.load("/scratch/dnlrea/pedro.romano/precompute_geom_au.npz")
...
results_dir = "results_S3_CNT75_Au"
basis_set = "def2-tzvp"
...
voltages = np.array([0, 250, 500], dtype=float)
...
np.savetxt("stabilization_energy_CNT(Au).txt", np.column_stack([E_fields[1:], stabilization_energies]),
           header="Field (V/m)    Stabilization Energy (eV)")

np.savetxt("work_function_Efield_CNT(Au).txt", np.column_stack([E_fields[1:], work_functions]),
...
with open("mulliken_charges_CNT_Au.txt", "w") as f:
...
plt.title("Mulliken Atomic Charges - CNT (Au)")
plt.tight_layout()
plt.savefig("mulliken_charges_barplot_Au.png", dpi=300)
plt.close()
print("  -> Mulliken charge barplot saved as MCharges_Au.png")
...
plt.grid(False)
plt.savefig("Stabilization_vs_Field_CNT(Au).png", dpi=500)
plt.close()
...
ax.yaxis.set_major_formatter(ticker.FuncFormatter(sci_fmt))
plt.grid(False)
plt.savefig("WorkFunction_vs_Field_CNT(Au).png", dpi=500)
plt.close()
