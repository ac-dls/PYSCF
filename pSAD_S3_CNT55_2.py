# SCF script: SCF convergence, WF, IP, Stabilization Energy and Mulliken Charges under E-field for CNT (5,5)
...
geom = np.load("/scratch/dnlrea/pedro.romano/precompute_geom_met.npz")
...
for i, (volt, E) in enumerate(zip(voltages, E_fields)):
    ...
    try:
        ...
        # Includes the use of chuncked data due to large number of electrons
        # ===============================================================
        ...
        for i0 in range(0, npts, CHUNK):
            ...
            # store if needed it later
            # exc_chunk[i0:i1] = exc_per_point   
        ...
        # replacing possible NaNs in components by local finite-means to avoid propagating NaN
        ...
        v_loc = v_hartree + vxc_grid + v_nuc_p
         ...
         # Fix: subtracting asymptotic Coulomb offset (finite-system gauge)
        ...
        # Computing average far-field electrostatic potential (Hartree + nuclear)
...
for j, E in enumerate(E_fields[1:]):  # skip the 0 field for stabilization
    ...
    try:
        ...
        # We saved vacuum per field earlier (vacuum_profile_field{E_int}.txt). Also, we re-load it to be robust:
        ...
        if os.path.exists(vac_file):
            try:
                ...
                # computing vacuum by averaging V_loc at the largest radii rows
                radii = data[:, 0]
                Vloc = data[:, -1]
                # picking rows where radius is within top 20%
        ...
        # If either vacuum_level or homo_E is not finite, skipping producing a work function
        ...
        # Saving work function file (Ha and eV) even if NaN 
        try:
        ...
        # ensuring E0 is set for stabilization energy
        if E0 is None:
        ...
        # Converting last appended values to eV (in-place)
...
np.savetxt("stabilization_energy_CNT(5,5).txt", np.column_stack([E_fields[1:], stabilization_energies]),
           header="Field (V/m)    Stabilization Energy (eV)")

np.savetxt("work_function_Efield_CNT(5,5).txt", np.column_stack([E_fields[1:], work_functions]),
...
with open("mulliken_charges_CNT_5,5.txt", "w") as f:
...
plt.close()
print("  -> Mulliken charge barplot saved as MCharges_55.png")
...
plt.grid(False)
plt.savefig("Stabilization_vs_Field_CNT(5,5).png", dpi=500)
plt.close()
...
ax.yaxis.set_major_formatter(ticker.FuncFormatter(sci_fmt))
plt.grid(False)
plt.savefig("WorkFunction_vs_Field_CNT(5,5).png", dpi=500)
plt.close()
